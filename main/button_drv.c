...
void short_press(int gpio) { ESP_LOGI(TAG, "Button %d short press", gpio); }

void long_press(int gpio) { ESP_LOGI(TAG, "Button %d long press", gpio); }
  ...
  for (; btn_target; btn_target = btn_target->next) {
    ...
    switch (btn_target->state) {
      ...
      case BUTTON_HOLD:  // 按住状态，如果时间长度超过设定的超时计数，将触发长按回调函数，进入BUTTON_LONG_PRESS_HOLD
        if (btn_target->btn_cfg.getlevel_cb(gpio_num) == btn_target->btn_cfg.active_level) {
          ...
          if (btn_target->press_cnt >=
              btn_target->btn_cfg.long_press_time)  // 已经检测到按下大于预设长按时间,执行长按回调函数
          {
            if (btn_target->btn_cfg.long_cb) btn_target->btn_cfg.long_cb(gpio_num);
      ...
      case BUTTON_LONG_PRESS_HOLD:  // 此状态等待电平消失，回到BUTTON_RELEASE状态
        if (btn_target->btn_cfg.getlevel_cb(gpio_num) !=
